...
## 背景及概念
 ...
 octo-ns
 是美团octo服务治理体系进行服务注册发现的套件，其基于zookeeper并由多个组件和服务组成，包括[c++ sdk](https://github.com/Meituan-Dianping/octo-ns/blob/master/mns-sdk/docs/mns_sdk_api.md)、[java sdk](https://github.com/Meituan-Dianping/octo-ns/tree/master/mns-invoker),
 [服务治理代理sgagent](https://github.com/Meituan-Dianping/octo-ns/tree/master/sg_agent)、[服务缓存nsc](https://github.com/Meituan-Dianping/octo-ns/tree/master/mns-cache)、[健康检查服务scanner](https://github.com/Meituan-Dianping/octo-ns/tree/master/scanner)
...
* 健康检查(health check)
  ...
  服务节点的端口/进程存活状态的检查。octo-ns体系中通过scanner执行，并将结果信息更新到存储中心zookeeper。

## 架构特点
* 去中心模块化架构

  通信框架(whale、dorado)、服务治理代理(sgagent)、健康检查服务(scanner)、服务治理平台(octo-portal)
...
* 容灾特点
   ...
   哨兵模式：当服务消费者或服务提供者连接本地sgagent不可用时，可通过连接到哨兵集群接入到octo-ns

##  架构介绍

<div align=center> <img src="./docs/image/ns_components.svg" width="500"> </div align=center>

   octo-ns由sgagent、nsc、zookeeper、scanner组成，使用octo-portal管理端实现可视化服务操作，如更改服务提供者状态、调整权重. 服务提供者通过通信框架及sdk接入octo-ns，进行服务注册和服务发现。
...
* 服务治理代理sgagent
  ...
  **标签过滤**: 对于服务提供者自定义了标签属性的服务发现流程，可按照传入标签返回对应感兴趣服务提供者列表
...
* 服务注册
   ...
   octo-ns支持使用c++/java sdk发起服务注册，可依赖本地或哨兵sgagent进行，在使用哨兵sgagent发起注册时，sdk可通过nsc服务获取远程sgagent哨兵机器列表信息，通过获得的哨兵机器节点发起服务注册流程，服务提供者注册数据会被写入到zookeeper中，从octo-portal服务提供者页面可查看到不同环境注册的服务提供者信息。
...
* [快速开始](https://github.com/Meituan-Dianping/octo-ns/blob/master/docs/ns-quick-start.md)
...
* 注册中心ap化
   ...
   以nsc为核心，将目前基于zookeeper的cp系统演进为ap系统，在保证最终一致性的前提下以可用性为优先目标
