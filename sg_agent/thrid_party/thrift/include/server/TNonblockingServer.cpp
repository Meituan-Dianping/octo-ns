...
class TNonblockingServer::TConnection {
 ...
 public:
  ...
  TConnection(int socket, short eventFlags, TNonblockingServer *s,
              const sockaddr* addr, socklen_t addrLen, int sgagentIndex) {
    readBuffer_ = NULL;
    readBufferSize_ = 0;
    sgagentIndex_ = sgagentIndex;  
...
void TNonblockingServer::TConnection::transition() {
  ...
  case APP_READ_REQUEST:
    ...
    server_->incrementSgagentHandlers(sgagentIndex_);
    ...
    if (server_->isThreadPoolProcessing()) {
      ...
      try {
        ...
        server_->decrementSgagentHandlers(sgagentIndex_);
...
TNonblockingServer::TConnection* TNonblockingServer::createConnection(
  ...
  if (connectionStack_.empty()) {
    return new TConnection(socket, flags, this, addr, addrLen, curSgagentIndex_);
  } else {
