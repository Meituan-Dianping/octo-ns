...
class TNonblockingServer : public TServer {
 private:
  ...
  /// Number of sgagent connection object we've created
  size_t numSgagentHandlers_[2];

  /// The sgagent is used
  int curSgagentIndex_;
  ...
  void init(int port) {
    ...
    eventBase_ = NULL;
    ownEventBase_ = false;
    numSgagentHandlers_[0] = 0;
    numSgagentHandlers_[1] = 0;
    curSgagentIndex_ = 0;
    numTConnections_ = 0;
 ...
 public:
  ...
  size_t getOldSgagentHandlers() {
    if (!curSgagentIndex_) {
      return numSgagentHandlers_[1];
    } else {
      return numSgagentHandlers_[0];
    }
  }

  void incrementSgagentHandlers(int sgagentIndex) {
    ++numSgagentHandlers_[sgagentIndex];
  }

  void decrementSgagentHandlers(int sgagentIndex) {
    --numSgagentHandlers_[sgagentIndex];
  }
